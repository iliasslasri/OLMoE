...
RUN_NAME             = "loss_free_balancing_sigmoid_u1.0_24H"
...
GLOBAL_TRAIN_BATCH_SIZE      = 8           # total tokens per optimizer step across all GPUs
...
MOE_GATE_TYPE                = "sigmoid"

# ── MoE Bias Update Speed (loss-free only) ──────────────────────────────────
# Step size u for the sign-based bias update: b[i] += u * sign(c_avg - c[i]).
# Wang et al. (2024) optimal: u = 0.001. Default in router.py: 1.0.
MOE_BIAS_UPDATE_SPEED        = 1.0
...
if not os.path.exists(OLMOE_DIR):
    ...
    # Update OLMoE repo (--no-recurse-submodules avoids stale submodule ref errors)
    # Use fetch + reset --hard to handle untracked files that now exist in the branch
    subprocess.run(["git", "fetch", "--no-recurse-submodules", "origin", OLMOE_BRANCH],
                   cwd=OLMOE_DIR, check=True)
    subprocess.run(["git", "checkout", OLMOE_BRANCH], cwd=OLMOE_DIR, check=True)
    subprocess.run(["git", "reset", "--hard", f"origin/{OLMOE_BRANCH}"],
                   cwd=OLMOE_DIR, check=True)
    # Update OLMo submodule directly (skip git submodule update which chases stale refs)
    subprocess.run(
        ["git", "config", "submodule.OLMo.url", OLMO_REPO_URL],
        cwd=OLMOE_DIR, check=True,
    )
...
cfg["model"]["moe_bias_update_speed"]    = MOE_BIAS_UPDATE_SPEED
...
print(f"  moe_bias_update_speed: {MOE_BIAS_UPDATE_SPEED}")
...
config_checks = [
    ...
    (verified_config["model"]["moe_bias_update_speed"] == MOE_BIAS_UPDATE_SPEED, "moe_bias_update_speed"),
...
print(f"  moe_bias_update_speed: {verified_config['model'].get('moe_bias_update_speed', 'N/A')}")
