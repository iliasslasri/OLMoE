...
LR_DISPLAY_ORDER = [3e-4, 1e-3, 3e-3, 8e-3, 2e-2, 6e-2, 2e-1]

# ── Manual overrides for optimal LR per d_model (set to None to auto-detect) ──
# Use these when runs are partially converged and the auto-argmin is noisy.
OPT_LR_OVERRIDE = {
    128: 3e-3,
}
...
def fetch_from_wandb(entity: str, project: str, group: str):
    ...
    for run in runs:
        ...
        final_loss  = (
            summary.get("train/CrossEntropyLoss")
            or summary.get("train/loss")
            or summary.get("train_loss")
            or summary.get("loss")
        )
        if final_loss is None:
            # Fallback: scan history for the last non-NaN loss
            try:
...
def plot(df_records: list, output_path: str):
    ...
    for d in dims:
        ...
        # Mark optimal (min loss, or manual override)
        if d in OPT_LR_OVERRIDE and OPT_LR_OVERRIDE[d] is not None:
            override_lr = OPT_LR_OVERRIDE[d]
            best_i = min(range(len(lrs)), key=lambda i: abs(lrs[i] - override_lr))
        else:
    ...
    if len(c_vals) >= 2:
        ...
        # Save for Phase 1 generate_grid_configs.py
        # lr*(C) = A_eta × C^(-beta_eta)  →  A_eta = exp(log_a),  beta_eta = -beta
        lr_params = {"A_eta": float(np.exp(log_a)), "beta_eta": float(-beta)}
        lr_params_path = os.path.join(os.path.dirname(output_path), "lr_params.json")
        import json
        with open(lr_params_path, "w") as _f:
            json.dump(lr_params, _f, indent=2)
        print(f"Saved LR params → {lr_params_path}  "
              f"(A_eta={lr_params['A_eta']:.4e}, beta_eta={lr_params['beta_eta']:.4f})")
...
def main():
    ...
    parser.add_argument("--group",   default="lr_sweep_v3")
